--[[lock Rings by londonali1010 (2009) Edited by jpope
...
function draw_clock_hands(cr,xc,yc)
    ...
    yh=yc-0.76*clock_r*math.cos(hours_arc)
...
function conky_clock_rings()
    local function setup_rings(cr,pt)
        ...
        if pt['arg'] == '%I.%M.%S' or pt['arg'] == '%M.%S' then
          -- second part, decimal part
          local strlen=string.len(pt['arg'])
          for i=strlen-1,3,-3 do
            str=string.format('${%s %s}', pt['name'], string.sub(pt['arg'], i, i + 1))
            str=conky_parse(str)
            -- both min(%M) and sec(%S) up to 60
            value=(value+tonumber(str))/60
          end

          local first_part=0
          -- first part, integral part
          str=string.format('${%s %s}', pt['name'], string.sub(pt['arg'], 1, 2))
          str=conky_parse(str)
          first_part=tonumber(str)
          -- '%I' range from 01 to 12, convert 12 to 00
          if string.sub(pt['arg'], 1, 2) == '%I' then
            first_part=first_part % 12
          end

          value=value+first_part
        else
